...
return  max;
       ...
       public int get_id_produit() throws SQLException {
          retrievedata a = retrievedata.getInstance("","",0);
        String query = "select p.id from produit p inner join commande_produit cp on p.id = cp.produit_id inner join commande c on c.id = cp.commande_id where c.user_id ="+a.getId()+" ";
Connection conn = getConnection();
...
return  max;
   ...
   //   c.setIdPanier(get_id_panier());
      String methode="";
        ...
        service.updateCommande2(methode, d.getId());
